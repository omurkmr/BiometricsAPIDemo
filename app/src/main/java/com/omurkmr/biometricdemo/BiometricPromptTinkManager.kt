...
class BiometricPromptTinkManager(private val activity: AppCompatActivity) {
    ...
    private fun saveEncryptedData(dataEncrypted: ByteArray) {
        val encryptData = Base64.encodeToString(dataEncrypted, Base64.DEFAULT)
        sharedPreferences.edit().putString(DATA_ENCRYPTED, encryptData).apply()
    ...
    private fun getEncryptedData(): ByteArray? {
        ...
        return when {
            data != null -> {
                val decryptData = Base64.decode(data, Base64.DEFAULT)
                decryptData
            }
            else -> null
        }

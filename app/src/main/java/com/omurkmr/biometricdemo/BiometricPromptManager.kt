...
class BiometricPromptManager(private val activity: AppCompatActivity) {
    ...
    private fun getEncryptedData(): ByteArray? {
        val dataEncrypted = sharedPreferences.getString(DATA_ENCRYPTED, null)
        return when {
            dataEncrypted != null -> Base64.decode(dataEncrypted, Base64.DEFAULT)
            else -> null
        }
    }
    ...
    private fun saveEncryptedData(dataEncrypted: ByteArray, initializationVector: ByteArray) {
        ...
        val dataEncrypt = Base64.encodeToString(dataEncrypted, Base64.DEFAULT)
        val iv = Base64.encodeToString(initializationVector, Base64.DEFAULT)
        sharedPreferences.edit().putString( DATA_ENCRYPTED, dataEncrypt) .apply()
        sharedPreferences.edit().putString(INITIALIZATION_VECTOR, iv).apply()
